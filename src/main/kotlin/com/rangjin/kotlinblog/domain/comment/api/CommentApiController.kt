...
class CommentApiController (
    ...
    fun create(@PathVariable id: Long, @RequestBody request: CommentCreateOrUpdateRequestDto): ResponseEntity<Any> {
        return ResponseEntity.ok().body(commentService.create(request, id))
    }

    @PutMapping("/update/{articleId}/{commentId}")
    fun update(@PathVariable articleId: Long, @PathVariable commentId: Long, @RequestBody request: CommentCreateOrUpdateRequestDto): ResponseEntity<Any> {
        return ResponseEntity.ok().body(commentService.update(request, articleId, commentId))
    }

    @DeleteMapping("/delete/{articleId}/{commentId}")
    fun delete(@PathVariable articleId: Long, @PathVariable commentId: Long, @RequestBody request: CommentDeleteRequestDto): ResponseEntity<Any> {
